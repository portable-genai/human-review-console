...
stack-trace-shaped nothing, and an operator got no reason. Executed against this repo before the
fix, ``GET /v1/reviews`` and ``GET /v1/cases`` under ``REVIEW_PROFILE=onprem`` both answered
``500 Internal Server Error``. That is the same defect as an unhandled failure anywhere else,
dressed as a portability placeholder.
...
``NotImplementedError``" is a claim the contract suite and ``scripts/portability_demo.py`` both
make about EVERY port. Both remain true at once.
"""
...
_MESSAGE = (
    ...
    "config.py under _IDENTITY_BINDINGS['onprem'] (see docs/onprem-migration.md), or run a "
    "profile whose identity adapter verifies an assertion."
)
...
class OnPremIdentityUnimplementedError(EndUserAuthUnavailableError, NotImplementedError):
    ...
    Two ancestries, both load-bearing. As an
    :class:`~review_console.ports.identity.EndUserAuthUnavailableError` it carries a status and a
    reason to the caller instead of a bare 500. As a :class:`NotImplementedError` it keeps the
    exit family's uniform refusal, which the contract suite and the portability demo assert for
    every port at once.
