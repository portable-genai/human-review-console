...
This repository has no separate parity-suite settings helper: the binding tables
(:data:`review_console.config._BINDINGS` and :data:`review_console.config._IDENTITY_BINDINGS`)
and the container that reads them ARE the wiring, and ``tests/test_contract_parity.py`` drives
the same two objects. So the probe builds the real container and touches every port the tables
name, rather than restating a construction loop that could drift from the one that ships. The
container asserts Protocol conformance as it binds, so a port that constructs into the wrong
shape is caught here too.
"""
...
def main(profile: str) -> int:
    ...
    # Every port the binding table names, plus identity, whose table is separate. Enumerated
    # from the table rather than listed here, so a port added to the hexagon is proved on
    # arrival instead of when somebody remembers this file.
    ports = [*sorted(_BINDINGS), "identity"]
    for port in ports:
