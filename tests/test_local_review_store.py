"""Durability and idempotency checks for the SDK-free local human-review-console review store."""
...
def test_source_key_is_idempotent_and_tenant_scoped_after_restart(tmp_path: Path) -> None:
    ...
    assert restarted.get_by_source_key(
        "other-bank", "cdd-sow-research:demo-bank:case-001:cdd_dossier"
    )
