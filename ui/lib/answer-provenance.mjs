...
// model: no header, no answer. (This service binds no model port today, so no response carries
// either header and the pill stays on the configured `no-model`.)
//
...
//
// Unlike the template, this console has no same-origin proxy: the browser calls the service
// directly at the base `lib/api-base.mjs` resolves, so that base is what "the console's own API"
// means here, and the service lists both headers in `Access-Control-Expose-Headers` so a
// cross-origin page may read them.
...
/**
 * True when `input` addresses this console's own service: the origin of `base`, under its path.
 *
 * @param {unknown} input what was handed to `fetch`
 * @param {string} here the page's own URL
 * @param {string} base the service base URL the rest of the console calls
 */
...
/**
 * Call `listener` with every answer the console's own service responses carry.
 *
 ...
 * @param {string} base the service base URL (`BASE_URL` in `lib/api.ts`)
