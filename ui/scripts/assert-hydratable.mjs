...
//
// Everything cheaper than this has been fooled by the defect it catches. The unit tests assert the
// CSP string, and the string was right. `tsc` was clean. `next build` succeeded. The page rendered,
// the headers were correct, and a screenshot looked exactly like a working console. What was
// actually shipped could still be dead markup: a `script-src` with no nonce blocks Next's inline
// hydration bootstrap, `__next_f` never fills, React never attaches, and no button does anything.
//
...
//
//   0. Every directive this console's posture depends on is present, and none of them is empty.
//      An empty directive is a parse error browsers discard, which silently removes the control.
...
// strictly MORE than an un-nonced policy did. Header and markup have to agree, and only the
// markup knows.
//
...
/** Directives whose ABSENCE is the fleet defect this console is being converged away from. */
...
// `cwd` is pinned to the ui/ directory so `npm --prefix ui run assert-hydratable` works from the
// repository root, where `next start` would otherwise find no app.
...
try {
  ...
  if (exited) {
    ...
    if (process.exitCode !== 1) {
      console.log(
        `OK every one of ${scriptTags.length} script tags carries the CSP nonce; the page hydrates.`,
      );
    }
  }
} finally {
